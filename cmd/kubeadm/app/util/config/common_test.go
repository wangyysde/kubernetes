...
func TestMigrateOldConfigFromFile(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			apiVersion: %s
			kind: ClusterConfiguration
			`, kubeadmapiv1old.SchemeGroupVersion.String(), kubeadmapiv1old.SchemeGroupVersion.String())),
			expectedKinds: []string{
				constants.InitConfigurationKind,
				constants.ClusterConfigurationKind,
			},
		...
		},
		{
			...
			discovery:
			  bootstrapToken:
			    ...
			    apiServerEndpoint: kube-apiserver:6443
			    unsafeSkipCAVerification: true
			`, kubeadmapiv1old.SchemeGroupVersion.String(), kubeadmapiv1old.SchemeGroupVersion.String(), kubeadmapiv1old.SchemeGroupVersion.String())),
			expectedKinds: []string{
				constants.InitConfigurationKind,
				constants.ClusterConfigurationKind,
				constants.JoinConfigurationKind,
